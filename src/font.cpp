  ...
  , m_characterSetBitmapMap(a_pIniFile)
{
  ...
  videoService.loadBitmap(m_characterSetBitmap, pCharacterSetBitmapFile);
...
Font::~Font() {
  VideoService& videoService = ServiceLocator::getVideoService();
  videoService.unloadBitmap(m_characterSetBitmap);
}

void Font::drawNumberOnScreen(int a_number, int a_x, int a_y, int a_nDigits) {
  VideoService& videoService = ServiceLocator::getVideoService();
  Rect characterSetBitmapArea = { 0, 0, m_characterHeight, m_characterWidth };
  if (a_number < 0) {
    characterSetBitmapArea.y = m_characterSetBitmapMap.getIntValue("-", "Characters", 0);
    videoService.copyToScreen(m_characterSetBitmap, a_x, a_y, &characterSetBitmapArea);
    a_number *= -1;
  }
...
void Font::drawTextOnBitmap(std::string a_text, Bitmap& a_destinationBitmap, int a_x, int a_y) {
  VideoService& videoService = ServiceLocator::getVideoService();
  Rect characterSetBitmapArea = { 0, 0, m_characterHeight, m_characterWidth };
  for (unsigned int iCharacter = 0; iCharacter < a_text.length(); iCharacter++) {
    ...
    videoService.copyToBitmap(m_characterSetBitmap, a_destinationBitmap, a_x, a_y, &characterSetBitmapArea);
    a_x += m_characterWidth;
  }
}

void Font::drawTextOnScreen(std::string a_text, int a_x, int a_y) {
  VideoService& videoService = ServiceLocator::getVideoService();
  Rect characterSetBitmapArea = { 0, 0, m_characterHeight, m_characterWidth };
  for (unsigned int iCharacter = 0; iCharacter < a_text.length(); iCharacter++) {
    if (a_text[iCharacter] == ' ') {
